...
export default function AddLP() {
  ...
  const calculateOptimalPair = (
    ...
    if (x_in) {
      const lp_ratio = Number(metadata.balance_y) / Number(metadata.balance_x);

      return amount * lp_ratio;
    } else {
      const lp_ratio = Number(metadata.balance_x) / Number(metadata.balance_y);
  ...
  return (
    <div className="">
      <Head>
        <title>BaptSwap | Create Pair</title>
      </Head>
      <MouseParallaxContainer
        globalFactorX={0.1}
        globalFactorY={0.1}
